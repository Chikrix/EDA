...
getColorRampAlpha <- function(n, alpha, ...) {
  colorRamp = colorRampPalette(...)(n)
  paste(colorRamp, sprintf("%x", ceiling(255 * alpha)), sep="")
}

image(volcano, col = getColorRampAlpha(20, 0.5, cols))
...
x1 <- rnorm(30)
y1 <- rnorm(30)
df <- data.frame(x1, y1, col_cat = rep(col_pal, each = 6))
plot(x1, y1, col = rep(col_pal, each = 6), pch = 19)
legend("topright", legend = paste("Group", 1:5), col = col_pal, pch = 19, bty = "n")
```
